...
<%
	...
	List<Bookmark> allBookmarks = myBookmarkService.getMarkedBookmarks(
...
<table id=filter>
	<tr>
		<td><b>Filter : </b><input name="filter"
			onkeyup="filter(this, 'MarkedBookmarksTable')" type="text">&nbsp;&nbsp;</td>
		<%
...
<hr>
<table id='MarkedBookmarksTable' height=100% width=100%>
	<%
		...
		<td>
		    ...
		    <%
 			...
 			<input type="button" class="utilButton" value="play"
				   id="4play<%=bookmark.getId()%>"
				   onclick="getVideo('4playVideo<%=bookmark.getId()%>','4play<%=bookmark.getId()%>','<%=vid%>');" />&nbsp;
			<div id="4playVideo<%=bookmark.getId()%>"></div>
