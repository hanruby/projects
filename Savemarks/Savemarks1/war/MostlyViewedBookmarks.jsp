...
<%
	...
	List<Bookmark> allBookmarks = myBookmarkService
			.getMostlyViewedBookmarks(user, category, hierarchy, tags);
%>
...
<table id=filter>
	<tr>
		<td><b>Filter : </b><input name="filter"
			onkeyup="filter(this, 'MostlyViewedBookmarksTable')" type="text">&nbsp;&nbsp;</td>
		<%
...
<hr>
<table id='MostlyViewedBookmarksTable' height=100% width=100%>
	<%
		...
		<td>
		    ...
		    <%
 			...
 			<input type="button" class="utilButton" value="play"
				   id="2play<%=bookmark.getId()%>"
				   onclick="getVideo('2playVideo<%=bookmark.getId()%>','2play<%=bookmark.getId()%>','<%=vid%>');" />&nbsp;
			<div id="2playVideo<%=bookmark.getId()%>"></div>
