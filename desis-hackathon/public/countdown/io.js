...
(function() {
  ...
  function getDigits() {
      ...
      var days = padNum(Math.floor(dateDiff/86400), 2);
      var timeRemaining = Math.floor(dateDiff%86400);
      var hours = padNum(Math.floor(timeRemaining/3600) + days * 24, 2);
      var minutes = padNum(Math.floor((timeRemaining%3600)/60), 2);
      var seconds = padNum(((timeRemaining%3600)%60)%60, 2);
      var values = [hours, minutes, seconds].join('').split('');
      values.splice(2,0,':');
      values.splice(5,0,':');
      //values.splice(8,0,':');
      return values;
  ...
  /**
    ...
    */
  var countdownTo = new Date(2012, 5, 17, 17, 30, 0).getTime();
